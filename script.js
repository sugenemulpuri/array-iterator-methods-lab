...
const fullName = inventors.map(inventor => ({firstName: inventor.first, lastName: inventor.last
}));
...
const sortedByBirthYear = inventors.sort((a, b) => a.year > b.year ? 1 : -1);
...
const isAdultPresent = devs.some(check18);
...
const isEveryone19OrOlder = devs.every(check19);
...
const checkId = ((comment) => comment.id  == 123523);

const idx = comments.findIndex(checkId);
